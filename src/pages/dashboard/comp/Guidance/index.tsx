import React, { useEffect, useState } from 'react';
import { Box, Flex, Text, Image } from '@chakra-ui/react';
import Button from '@/components/Button';
import api from '@/lib/api';
import ImgArrowUp from '@/assets/icons/arrow-up.svg';
import { useSlotStore } from '@/store/slot';
import { motion } from 'framer-motion';
import useTools from '@/hooks/useTools';
import { useSettingStore } from '@/store/setting';

export default function Guidance() {
  const { slotInfo } = useSlotStore();
  const { setFinishedSteps, collapseGuidance, toggleCollapseGuidance } = useSettingStore();
  const { goGuideAction } = useTools();
  const checkSteps = async () => {
    if (!slotInfo.slot) {
      return;
    }
    const res = await api.operation.finishStep({
      slot: slotInfo.slot,
      steps: [],
    });

    setFinishedSteps(res.data.finishedSteps);
  };

  useEffect(() => {
    checkSteps();
  }, []);

  return (
    <Box
      px="6"
      pb="2px"
      w={{ base: '380px', '2xl': '400px' }}
      maxW={'95%'}
      mt="-2"
      bg="#fff"
      border="1px solid #EAECF0"
      boxShadow={'0px 4px 60px 0px rgba(44, 53, 131, 0.08)'}
      roundedBottomLeft={'20px'}
      roundedBottomRight={'20px'}
      overflow={'hidden'}
    >
      <Box
        overflow={'hidden'}
        as={motion.div}
        h={collapseGuidance ? 0 : 'auto'}
        animate={{ height: collapseGuidance ? 0 : 'auto' }}
      >
        <Text fontSize={'18px'} fontWeight={'800'} lineHeight={'1.25'} mt="7" mb="3">
          Active your Soul Wallet
        </Text>
        <Flex align={'center'} justify={'space-between'} gap="8">
          <Text fontSize={'12px'} lineHeight={'1.5'}>
            Add native assets and complete the first transaction to active your wallet.
          </Text>
          <Button
            boxSizing="content-box"
            px="5"
            py="2"
            fontWeight={'700'}
            fontSize={'12px'}
            onClick={() => {
              goGuideAction(-1);
            }}
            bg="brand.black"
            color="white"
            _hover={{ bg: 'brand.purple', color: 'white' }}
          >
            Activate
          </Button>
        </Flex>
      </Box>
      <Box textAlign={'center'} cursor={'pointer'} onClick={() => toggleCollapseGuidance()}>
        <Image
          src={ImgArrowUp}
          transform={collapseGuidance ? 'rotate(180deg)' : 'rotate(0deg)'}
          mt={collapseGuidance ? 2 : 3}
          mx="auto"
          display={'block'}
        />
      </Box>
    </Box>
  );
}
